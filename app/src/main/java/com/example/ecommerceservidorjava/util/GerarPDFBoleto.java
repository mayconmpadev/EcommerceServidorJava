...
public class GerarPDFBoleto extends AppCompatActivity {
    ...
    public GerarPDFBoleto(Boleto boleto, Context context) {
        this.boleto = boleto;
    ...
    private String total(String tipo, int valor) {
        ...
        for (int i = 0; i < boleto.getIdVenda().getItens().size(); i++) {
            if (boleto.getIdVenda().getItens().get(i).getQtd() != 0) {
                BigDecimal preco = Util.convertMoneEmBigDecimal(boleto.getIdVenda().getItens().get(i).getPreco_venda());
                preco = preco.divide(new BigDecimal("100"));

                total = total.add(new BigDecimal(boleto.getIdVenda().getItens().get(i).getQtd()).multiply(preco));
    ...
    private void finalizar() {
        ...
        try {
            ...
            if (file.exists()) {
                createPdf(boleto, perfilEmpresa);
            } else {
    ...
    private void createPdf(Boleto boleto, PerfilEmpresa perfilEmpresa) throws IOException, DocumentException {
        ...
        Paragraph p11 = new Paragraph(10, Timestamp.getFormatedDateTime(Long.parseLong(boleto.getData()), "dd/MM/yy"), paragraphFont4);
        ...
        Paragraph p15 = new Paragraph(10, boleto.getData(), paragraphFont4);
        ...
        Paragraph p17 = new Paragraph(10, boleto.getIdVenda().getIdCliente().getNome(), paragraphFont);
        Paragraph p18 = new Paragraph(10, "Vendedor:", paragraphFont2);
        Paragraph p19 = new Paragraph(10, boleto.getIdVenda().getIdUsuario().getNome(), paragraphFont);
        ...
        Paragraph p21 = new Paragraph(10, boleto.getIdVenda().getIdCliente().getTelefone1(), paragraphFont);
        Paragraph p22 = new Paragraph(10, "Telefone 2:", paragraphFont2);
        Paragraph p23 = new Paragraph(10, boleto.getIdVenda().getIdCliente().getTelefone2(), paragraphFont);
        Paragraph p24 = new Paragraph(10, "Email:", paragraphFont2);
        Paragraph p25 = new Paragraph(10, boleto.getIdVenda().getIdCliente().getEmail(), paragraphFont);
        ...
        p1.add(boleto.getIdVenda().getSubTotal());
        document.add(p1);

        if (Integer.parseInt(boleto.getIdVenda().getDesconto()) > 0) {

            Chunk glue2 = new Chunk(new VerticalPositionMark());
            Paragraph p2 = new Paragraph("Desconto aplicado nos produtos", paragraphFont3);
            p2.add(new Chunk(glue2));
            p2.add(boleto.getIdVenda().getDesconto() + "%");
            document.add(p2);
        }
        ...
        p.add(boleto.getIdVenda().getTotal());
        document.add(p);
        ...
        p3.add(boleto.getIdVenda().getTipoPagamento());
        document.add(p3);
    ...
    private void createTable(Boleto boleto)
...
//l
        for (int i = 0; i < boleto.getIdVenda().getItens().size(); i++) {
            ...
            table.addCell(new PdfPCell(new Phrase(boleto.getIdVenda().getItens().get(i).getCodigo(), paragraphFont2)));
            table.addCell(new PdfPCell(new Phrase(boleto.getIdVenda().getItens().get(i).getNome(), paragraphFont2)));
            table.addCell(new PdfPCell(new Phrase(String.valueOf(boleto.getIdVenda().getItens().get(i).getQtd()), paragraphFont2)));
            table.addCell(new PdfPCell(new Phrase(boleto.getIdVenda().getItens().get(i).getPreco_venda(), paragraphFont2)));
            table.addCell(new PdfPCell(new Phrase(NumberFormat.getCurrencyInstance().format(somatoriaDosProdutosIguais(boleto.getIdVenda().getItens().get(i).getPreco_venda(), String.valueOf(boleto.getIdVenda().getItens().get(i).getQtd()))), paragraphFont2)));
        }
